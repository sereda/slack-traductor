...
function translate({ fromLang, toLang }, text) {
  ...
  if (!langCodes.has(fromLang)) {
    ...
    throw Error('Won\'t translate from [' + fromLang + ']');
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    request.get(
      ...
      (err, res, body) => err ? reject('' + err + ' : ' + JSON.stringify(res)) : resolve(body))
  })
  .then(trimMicrosoft);
}
...
function isLang(langCode, text) {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.get(
      ...
      (err, res, body) => {
        if (!err) console.log('detect => ' + body);
        err ? reject('' + err + ' : ' + JSON.stringify(res)) : resolve(body);
      })
  })
  .then(trimMicrosoft)
  .then((code) => ({
...
function trackExpiration() {
  setInterval(() => {
    isAccessable = false;
    updateAccess();
  }, 600 * 1000); // refresh every 10 minutes
}
